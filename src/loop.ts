...
export async function toolLoop(options: LoopOptions): Promise<LoopResult> {
  ...
  for (;;) {
    ...
    for (const [index, block] of message.content.entries()) {
      ...
      // Position, not `block.id`, here and in the driver (src/agents/driver.ts,
      // lesson 5.1), which keys its calls the same way off the step counter it
      // persists. A resumed turn asks the model the same questions and gets
      // fresh toolu_ ids back for the same calls, whether the transcript it
      // asks from lived in this local array or in course.turns.state: the
      // REQUEST is what a persisted transcript makes identical, never the
      // response.
      //
      // So: an id from the reply cannot recognise a call we already made.
      ...
      try {
        ...
        // the turn ends 'ambiguous_tool_call' and a person decides.
        // Deliberately its own reason, not 'fenced': fenced means another
        // worker holds the claim and this worker's own write was refused,
        // which is safe to leave alone because whoever fenced it is alive and
        // will finish it. This worker still owns the turn perfectly well; the
        // problem is out in the world, and it is the one case in this module
        // that genuinely needs a person, not a retry (src/repo/toolCalls.ts).
        if (!(err instanceof AmbiguousToolCallError)) throw err
        return finish('ambiguous_tool_call', '')
      }
