 ...
 * about $0.99 across its sixty-nine model calls, and a ceiling of $1 stopped it
 * twenty-four calls short of the end of its own recording. What stopped it was
 * not the spend, which never reached $1, but the RESERVATION: the driver reserves
 * an upper bound before every call (src/agents/driver.ts) and a turn's
 * reservation was what crossed. $4 is chosen against that, so the case that
 * spends $0.99 keeps its headroom, at about four times its spend. A number that
 * generous is not a cost control, it is a runaway stop, and the cost control is
 * the global ceiling below.
 *
