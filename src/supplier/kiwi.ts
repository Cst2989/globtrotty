...
/**
 * Baggage is load bearing for a recommendation: a 30 EUR fare with no cabin bag
 * is not cheaper than a 55 EUR fare with one. The fixture always carries the
 * object, and the field is typed as required, so a response that omits it would
 * produce `detail.baggage === undefined`, which reads downstream as "we have no
 * idea" and renders as nothing at all. Zero is the honest floor: it claims no
 * allowance we were not told about, and it makes the comparison conservative
 ...
 * and the only way `npm test` stays keyless.
 */
export function parseKiwiResponse(body: string, params: FlightSearch, now: Date): SupplierItem[] {
  ...
  // The tool result is a JSON STRING inside the JSON-RPC envelope, so it is
  // parsed twice.
  const data = JSON.parse(text) as { currency?: string; itineraries?: KiwiItinerary[]; error?: unknown }
  ...
  // The obvious guard, `if (data.currency && ...)`, treats a missing field as
  // agreement, which is "absence of evidence is confirmation": the requested
  // code would be stamped onto whatever number came back. This branch takes the
  // same position the rest of the codebase takes, because unknown is not
  ...
  return (data.itineraries ?? []).map((it) => {
    ...
    // lose a cent on every such fare.
    ...
    return {
      ...
      detail: {
        ...
        // request. Kiwi's virtual interlining defaults to TRUE and a missed
        // connection is then the traveller's problem, so this has to travel
        // with the item rather than be assumed.
 ...
 * which silently shifts every downstream date-window comparison by hours and
 * can move a 23:59 departure onto the next day.
 */
function leg(l: KiwiLeg): LegSummary {
  return {
    ...
    carriers: [...new Set((l.segments ?? []).map((s) => s.carrier ?? s.carrierName ?? '').filter(Boolean))],
    // Segment ORDER, not a deduplicated set. `carriers` above dedupes because
    // "who flies this" is a set; a flight number is per-segment identity and
    // two segments on one carrier have two of them. Empty entries are dropped
    // rather than kept as '' so the list holds only numbers we were told.
...
export const KIWI_CAPABILITIES: SupplierCapabilities = {
  live: true,
  // Verified against the live endpoint when the fixture was captured: itinerary
  // ids are stable across repeated identical searches, so quote() is a real
  // re-search and find rather than a claim.
  mayRequote: true,
  // Shared with MockSupplier's freshness window, see DEFAULT_MAX_AGE_SECONDS in
  // types.ts. Not a fresh literal: one definition, two consumers.
...
export class KiwiSupplier implements Supplier {
   ...
   * not verify, and unknown is NOT unchanged, so that surfaces as `unavailable`
   * and blocks the hand-off (lesson 4.6) rather than passing quietly.
   */
