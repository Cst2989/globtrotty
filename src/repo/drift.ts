...
/**
 * The full (unreduced) `request_shape` from the newest `model_calls` row for
 * this seat that has one at all — `capture_policy` writes NULL for a
 * truncated/sampled-out row (src/repo/modelCalls.ts), and this is scoped to
 * the three seats that are always `'full'`, so `is not null` is really
 * "the newest row", but stated explicitly rather than assumed.
 */
export async function newestRequestShape(
  ...
  const rows = await sql<{ request_shape: unknown }[]>`
    select request_shape from model_calls
     where seat = ${seat} and request_shape is not null
...
/**
 * The ops conversation every canary call reserves/reconciles/records
 * against, so its counters never touch a traveller's. Reuses the newest one
 * on record rather than minting a fresh row every night — `reserve` requires
 * a `conversations` row for the user (src/repo/reservation.ts), and one
 * durable ops conversation is what makes its running total ("summed canary
 * costs") a meaningful figure rather than a fresh zero every run.
 */
export async function ensureOpsConversation(sql: postgres.Sql): Promise<string> {
  const existing = await sql<{ id: string }[]>`
    select id from conversations where user_id = ${OPS_USER_ID} order by created_at desc limit 1`
  const row = existing[0]
  if (row !== undefined) return row.id
  const created = await sql<{ id: string }[]>`
    insert into conversations (user_id, desk) values (${OPS_USER_ID}, 'planning') returning id`
  return created[0]!.id
}
