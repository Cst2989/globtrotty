...
## Plan 3b — carried debt after the gates half

Recorded at plan 3b's Task 12, the same pass that wrote its decisions doc
(`superpowers/2026-09-13-plan-3b-gates-decisions.md`). Numbered `3b.N`, continuing the file's
tier-numbering style rather than reusing Tier 3's or Tier 4's own sequences.

| # | Item | Notes |
|---|---|---|
| 3b.1 | No reaper for `link_clicks` or `escalations` | Both tables are append-only with no retention job, the same shape as 2.4's `tool_results`/`model_calls` finding — verified there is no `pg_cron` install and no `%reap%`/`%retention%`/`%purge%`/`%prune%` function in this database, and that check covers these two tables too. Retention for either is unspecified by the spec. Cheap while both tables are near-empty; size a reaper (or a written "no reaper, and why" decision) alongside 2.4's. |
| 3b.2 | `Notifier` has no real transport | The only implementation shipped is `LogNotifier` — it logs and returns. §7's escalation effect is real (the row, the sticky `'escalated'` status, the rate limit) but nothing pages an actual human yet. A real adapter (email, SMS, on-call) is a port implementation away; `DriverDeps.notifier` is already the seam. |
| 3b.3 | `gt_ref` is a placeholder affiliate parameter | `TRACKING_PARAM = 'gt_ref'` in `src/supplier/urls.ts`, with a comment noting the name is "ours to choose until an affiliate programme is joined". No programme exists yet, so the parameter carries no real affiliate id — it identifies the click for `link_clicks`/`conversions` bookkeeping only. Renaming it once a real programme is joined is a one-constant change, not a schema change. |
| 3b.4 | `spent.micros` and `recordedMicros` are both lost on process death of an attempt | Both accumulators live in memory for the life of one attempt and are folded into `turns.spend_usd_micros` only when that attempt returns (`recordedMicros`) or throws in a way the calling frame can catch (`spent.micros`, now covered by Task 12's F4 fix, which moved the fold into a `finally`). Neither survives the process itself dying — a killed worker, an OOM, a hard crash — between a provider debit and the fold. This is the same class of gap as the harness's crash-recovery story generally (a resumed turn re-reads persisted rows, not in-memory accumulators), not a new one 3b introduced, but 3b is the first plan to name `spent.micros` alongside the pre-existing `recordedMicros` gap. |
| 3b.5 | `markNotified` failure after a successful notify fails the turn | `escalate_to_human` pages the human successfully, then a failed follow-up write to set `notified_at` fails the whole turn — the escalation row and the page both already landed, only the turn's own completion and her reply are lost. Fix is a one-line `.catch` around the `markNotified` call so a failure there does not fail an otherwise-successful escalation. Recorded in the decisions doc as a parked residual from the final review. |
| 3b.6 | The sweeper's crash-loop reap overwrites `'escalated'` | `sweeper.ts` sets `conversations.status = 'failed'` unconditionally when it reaps a crash-looping turn, the same class of status overwrite Task 12's F1 fixed inside `completeTurn`/`failTurn`. The sweeper is plan 1 code, outside plan 3b's file map, so the fix was parked rather than made here. An escalated conversation that also crash-loops currently reads `'failed'` instead of `'escalated'`. |
| 3b.7 | `ambiguous` could safely re-run `hand_off_to_booking` | `hand_off_to_booking` mints `link_clicks` idempotently by `proposal_id` — a second run after an `ambiguous` resume would find the existing rows and return them rather than minting a duplicate set. Letting `ambiguous` re-run the tool (instead of the worker unconditionally failing the turn `fenced`) would close the parent-spec-§5.6 tension the decisions doc names under "Deviation 1 conflicts with the parent spec §5 point 6". Not done here because it is a change to how the worker treats every `pending` `tool_calls` row on resume — a harness-wide rule, not a change local to one tool. |
| 3b.8 | `decideProposal` has no production caller until plan 4's route | Plan 4's chat UI route handler is specified to call `decideProposal` from her accept/reject button (§5 of this plan). Until that route exists, the only callers are `scripts/demo.ts` and tests — the cashier's precondition (`decision = 'accept'` within 30 minutes) can never be satisfied outside a demo or a test, so `hand_off_to_booking` is unreachable in production. Not a defect; the seam is plan 4's, named here so it is not mistaken for one. |
| 3b.9 | No driver-level test through the `revise_component` switch case | Every other tool has a driver-level test exercising its `case` in `src/agents/driver.ts`'s `execute` switch; `revise_component`'s logic is well-tested at the module level (`src/tools/revise.ts`) but the switch case itself is not. Deferred at Task 6's review as a minor. |
| 3b.10 | The zod caps on reviewer issues are invisible to the model | `ReviewResult.issues` is capped at 20 items / 500 chars each by the zod schema the reviewer's structured output is validated against, but the model is never told the cap exists. An over-long legitimate rejection does not fail loudly with "too long" — it fails the generic shape-parse path and reads back as a synthetic "malformed" issue, same as a genuine parse failure. Deferred at Task 4's review as a minor. |
| 3b.11 | Plan 3c scope | Front desk (Haiku, structured output, routes to planning on any parse failure), destination scouts (read-only, fenced results), the drift monitor (unblocked by migration 0012, still needs a decision on cheap-seat sampling per backlog 2.5), CI (backlog 3.9, needs a non-production `DATABASE_URL`), and `trimForContext`'s price half (needs the cashier's re-quote path, now built) — all explicitly out of plan 3b's scope per this plan's own header, carried forward as plan 3c's task list rather than backlog debt. |

---

## Plan 3c — carried debt after the seats half

Recorded at plan 3c's Task 11, the same pass that wrote its decisions doc
(`superpowers/2026-09-13-plan-3c-seats-decisions.md`). Numbered `3c.N`, continuing the file's
tier-numbering style rather than reusing Tier 3's, Tier 4's, or 3b's own sequences.

| # | Item | Notes |
|---|---|---|
| 3c.1 | `titler` seat unused | The front desk's one structured-output call produces both the routing label and the title; `titler` stays declared in the `model_calls.seat` constraint, spare. Recorded in the plan header as a deliberate deviation, not a defect. |
| 3c.2 | Parallel scouts not built | The parent spec's prose describes three parallel scout dispatches; the loop answers one `tool_use` per step by construction (plan 3's own shape). `research_destination` is one worker-door tool, called once per driver step. A future plan that gives the loop multi-tool-call-per-step handling could parallelize this. |
| 3c.3 | `check_transfers` not built | No data source was ever chosen for it. Out of scope per the plan header. |
| 3c.4 | Scout shape drift is invisible | The drift monitor's shape check (§4) only covers the three full-capture seats (`driver`, `reviewer`, `front_desk`); `scout`'s `request_shape` is NULL whenever its combined system+user bytes exceed the truncation threshold (backlog 2.5), so there is nothing to diff for `scout` even though it is canaried. Ruled at plan time: canary-only coverage for cheap seats. |
| 3c.5 | CI's database roles differ from Supabase's | `.github/workflows/test.yml` runs a bare `postgres:16` service container with only the `anon`/`authenticated` roles the migrations assume (`supabase/ci-bootstrap.sql`) — there is no real Supabase project behind CI. Plan 4's forced-RLS policies need a Supabase-shaped test run to be proven correct; the CI container alone cannot exercise Supabase-specific RLS/role behavior. |
| 3c.6 | `run-turn-background.mts` still runs `echoAgent` | Carried forward from Tier 4 (pre-existing) — plan 3c did not touch this. Plan 4 is specified to wire `routeAgent` (this plan's front-desk/driver router) in as the production entry point's real agent, replacing the echo stand-in. |
| 3c.7 | Web search cost per scout is not separately tracked | Every scout call's web-search spend (the flat `WEB_SEARCH_MICROS` fee plus result tokens) is folded into the same `model_calls.cost_micros`/`turns.spend_usd_micros` figures as the rest of the call — there is no separate "search spend" line anywhere. Fine today (money is still correctly debited and ceilinged); a future cost-reporting feature that wants "search spend vs. token spend" per seat would need a new column or a derived query against `model_calls.request_shape`/`usage`. |
| 3c.8 | Golden prompts must change when a seat's prompt version changes — the first run after a bump has no comparison row | The drift monitor's shape check is now era-keyed (`prompt_version` + `model_config_id`, F2 fix) specifically so a version bump does not file a guaranteed false positive — but the practical consequence is that the first nightly run after any prompt or model-config bump has zero real (non-ops) rows in the new era to compare against, so `newestRequestShape` returns `null` and the shape check silently skips that seat for one night. This is correct behavior, not a defect, but it means an operator should not read "no shape alarm the night after a deploy" as "the deploy shipped exactly what the repo says" — it may just mean nothing has been diffed yet. |
| 3c.9 | `suppressed` count not logged/returned anywhere visible | `runDriftMonitor`'s `suppressed` field (alarms deduped within the 7-day window) is computed and returned from the function, but nothing in `drift-monitor.mts` logs it anywhere an operator would see it outside a direct unit-test call. A fully-suppressed night reads as "alarms: 0" with no visible signal that real (but already-known) drift existed. |
| 3c.10 | The monitor's own ceiling-skip alarm bypasses the 7-day dedupe | The `seat: 'monitor', check: 'canary', detail: { skipped, reason: 'ceiling' }` alarm filed when a seat is skipped for a ceiling reason is not itself deduplicated the way a seat's own canary/shape alarm is — a persistent ceiling problem (e.g. the ops conversation near its monthly cap) pages every single night rather than once. |
| 3c.11 | Alarm dedupe can silence a re-occurring flap | The 7-day byte-identical dedupe treats "the same detail already alarmed within 7 days" as fully suppressed, with no distinction between "still continuously broken" and "recovered, then broke again with an identical detail." A seat that flaps — drifts, gets fixed, drifts identically again within the window — would not re-page on the second occurrence. |
| 3c.12 | Scout search error codes unmasked/dropped when the brief is empty | `researchDestination`'s `searchErrorTrailer` appends failed web-search error codes to the brief unmasked (they are provider-generated error-code strings, not free-form supplier prose, which is why this was accepted rather than fixed), and is never reached at all on the "no brief" early-return paths (spending-limit reached, refusal, empty brief) — a search failure alongside one of those conditions is silently dropped rather than surfaced. |
| 3c.13 | L1 live-pin prompt-file reading duplicates `src/agents/*.ts` | `test/driver.live.test.ts`'s live pins re-read the same prompt files (`front_desk.md`, `scout.md`) via their own `readFileSync` calls rather than importing the constant each agent module already loads at its own top level. Harmless duplication, not a behavior risk. |
| 3c.14 | Drift tests' fixed clock sits at a distance from the 20-hour recency guard | `test/drift.test.ts` uses fixed `now()`/`ranAt` fixtures (and `monitor/drift.ts`'s `GOLDEN_NOW` for the reviewer's golden offer) that do not track wall-clock time the way the 20-hour `RECENT_CANARY_HOURS` guard (F6) does in production. Accepted as a test-authoring convenience; if the guard's window is ever tightened, the fixed-clock tests should be revisited to confirm they still discriminate the boundary correctly. |
| 3c.15 | "120 per day" no longer redacted by `redactPrices` | M9 (final fix wave) dropped `'day'`, `'week'`, and `'each'` from the `UNIT` list because a bare count before one of those words has no reliable price reading ("4 per day", "runs 9 to 5 each day"). Accepted trade: a legitimate price phrased that way (rare) would now survive redaction, in exchange for not over-redacting ordinary counts (common). |
| 3c.16 | The dated Haiku model id (`claude-haiku-4-5-20251001`) is a watch item | Named at the final review as item 15: it is the one current model with a dated snapshot (per the "API drift found while scoping plan 3" section below), so it is the one seat id that could silently go stale if a future Haiku release deprecates that exact snapshot. No action needed today; watch for a deprecation notice. |
| 3c.17 | The Anthropic key's credit balance is too low | Every `LIVE_MODEL=1` run against the real API — including pre-existing tests unrelated to this branch — fails identically with a 400, "Your credit balance is too low to access the Anthropic API." The key authenticates; the account has no spendable balance. Blocks: the L1 live pins (front desk, scout) from being verified live; CI's live suites (`LIVE_MODEL=1 LIVE_SUPPLIERS=1 pnpm test`) from succeeding on the first PR run; and the nightly drift monitor from making a single real canary call in production. Purely operational — the fix is topping up the account, not a code change. |
| 3c.18 | The PR/CI acceptance run has not happened yet | Task 10's CI pipeline is complete locally but its own acceptance criterion — the workflow running green on the PR that adds it — needs a push, an opened PR, and the two repo secrets (`ANTHROPIC_API_KEY`, `GOOGLE_SEARCH_API`) added by a human with repo-settings access. None of that has happened as of this writing. |
