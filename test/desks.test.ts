...
describe('desks', () => {
  it('gives the front desk no tools at all', () => {
    expect(toolsFor(loadDesk('front'))).toEqual([])
    expect(DESK_TOOLS.front).toEqual([])
  })
  it('lets the planning desk search and nothing else', () => {
    expect(toolsFor(loadDesk('planning')).map((t) => t.name)).toEqual(['search_flights', 'search_hotels'])
  })
